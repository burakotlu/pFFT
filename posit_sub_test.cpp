...
void writeToFile(const std::vector<double>& vec, const std::string& filename) {
    ...
    outFile.close();
    std::cout << "Div written to " << filename << std::endl;
}
...
int main() {
  ...
  /*  
    ...
    double a = randomArray[0];
    double b =  randomArray2[0];
    double c = a/b;
    ps_t a_p = double2posit(a);
    ps_t b_p = double2posit(b);
    ps_t c_p = positSub(a_p,b_p);
    ...
    std::vector<double> reconstructedArray = convertPositArrayToDouble(subPosit, "subPosit.txt");
    ...
    // Compute SNRs
    double snrPosit = computeSNR(subDouble, reconstructedArray);
    double snrFloat = computeSNRFloat(subDouble, subFloat);
